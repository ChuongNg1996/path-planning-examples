...
padding_dist = 3;   % pad the image with number of pixel
map = padarray(map,[padding_dist padding_dist],free_val,'both'); 

if in == 1
    in2 = input("Auto or Manual? Type 1 for auto, 0 for manual ");
    
    if in2 == 1
        clear_dist = 2;         % Number of pixel to check from an occupied pixel 
        % Go through each pixel of entire map
        for i = (padding_dist+1):map_wth 
            for j = (padding_dist+1):map_len
...
%% Define safe region
in = input("Define safe regions? Type 1 for yes, 0 for no.");
if in == 1
    safe_size = 0.2;  % size of the box in (m)
    safe_pixel = safe_size/pixel_size;      % Translate box_size to pixels
                                            % this should always smaller
                                            % than number of padding 
                                            
    [obs_row, obs_col] = find(map == obs_val); % find all obstacles
    obs_num = size(obs_row);
    for i = 1:obs_num
        % amplify the obstacle
        map((obs_row(i) - safe_pixel : obs_row(i) + safe_pixel),...
        (obs_col(i) - safe_pixel : obs_col(i) + safe_pixel)) = obs_val;
    end
